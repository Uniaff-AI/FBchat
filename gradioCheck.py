...
# Gradio Interface logic for the chatbot interaction
def chatbot_interface(user_input, chat_history):
    current_stage = len(chat_history)  # The stage advances with each user input
    stages = [
    ...
    if contains_action_tokens(user_input):
        try:
...
with gr.Blocks() as demo:
    gr.Markdown("## Product Assistant Chatbot")
    chatbot = gr.Chatbot(label="Chat with Product Assistant")
    user_input = gr.Textbox(label="Your input")
    state = gr.State([])


    def user_interaction(user_input, state):
        state, response = chatbot_interface(user_input, state)
        return state, state


    user_input.submit(user_interaction, [user_input, state], [chatbot, state])
